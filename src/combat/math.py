...
async def real_stat(
    stat_func: Coroutine[Any, Any, List[float]], uni_func: Coroutine[Any, Any, float]
) -> List[float]:
...
def spell_effect_stacking_id(
    spell_template_id: int, enchantment_spell_template_id: int
) -> tuple:
...
async def base_damage_calculation_from_id(
    ...
    caster_effects: List[DynamicSpellEffect] = await get_total_effects(
        caster_id, members
    )
    ...
    target_effects: List[DynamicSpellEffect] = await get_total_effects(
        target_id, members
    )
    ...
    caster_damages = await real_stat(
        caster_stats.dmg_bonus_percent, caster_stats.dmg_bonus_percent_all
    )

    caster_flat_damages = await real_stat(
        caster_stats.dmg_bonus_flat, caster_stats.dmg_bonus_flat_all
    )

    caster_crits = await real_stat(
        caster_stats.critical_hit_rating_by_school, caster_stats.critical_hit_rating_all
    )

    caster_pierces = await real_stat(
        caster_stats.ap_bonus_percent, caster_stats.ap_bonus_percent_all
    )

    caster_level = await caster.level()

    target_resistances = await real_stat(
        target_stats.dmg_reduce_percent, target_stats.dmg_reduce_percent_all
    )

    target_flat_resistances = await real_stat(
        target_stats.dmg_reduce_flat, target_stats.dmg_reduce_flat_all
    )

    target_blocks = await real_stat(
        target_stats.block_rating_by_school, target_stats.block_rating_all
    )
    ...
    for effect_atr in caster_effect_atrs:
        stacking_id = spell_effect_stacking_id(
            effect_atr.spell_template_id, effect_atr.enchantment_spell_template_id
        )
