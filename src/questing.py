...
from src.autopet import auto_pet

from src.lang.client import walk_quest_gate_if_cross_zone, run_tp_to_quest
...
class Quester:
    def __init__(self, client: Client, clients: list[Client], leader_pid: int):
        ...
        self._chunk_cache: dict[str, list[XYZ]] = {}

        self._npc_missing_iterations = 0
    ...
    async def read_spiral_door_title(self, client: Client) -> str:
        ...
        try:
            title_text_path = await get_window_from_path(
                client.root_window, spiral_door_title_path
            )
    ...
    async def read_popup(self, p: Client) -> str:
        ...
        try:
            popup_text_path = await get_window_from_path(
                p.root_window, popup_msgtext_path
            )
    ...
    async def detected_interact_from_popup(self, p: Client) -> bool:
        ...
        if "to talk" not in msg and "to open" not in msg and "to collect" not in msg:
            return True
    ...
    async def is_position_safe(
        self, position: XYZ, safe_distance: float = 1000
    ) -> bool:

        sp = EntityClient(self.client)

        for mob in await sp.mobs():
    ...
    async def get_cached_zone_chunks(self, client: Client) -> list[XYZ]:

        zone = await client.zone_name()

        cached = self._chunk_cache.get(zone)

        if cached is not None:
            return cached

        try:
            wad = await self.load_wad(zone)

            nav_data = await wad.get_file("zone.nav")

            vertices, _ = parse_nav_data(nav_data)

            chunks = calc_chunks(vertices)

        except Exception:
            chunks = []

        self._chunk_cache[zone] = chunks

        return chunks

    async def streaming_reset(self, clients: list[Client]) -> bool:

        chunks = await self.get_cached_zone_chunks(self.current_leader_client)
        ...
        farthest = max(
            chunks,
            key=lambda c: (c.x - leader_pos.x) ** 2 + (c.y - leader_pos.y) ** 2,
        )

        target = XYZ(farthest.x, farthest.y, farthest.z - 550)

        logger.debug(
            "Streaming reset: teleporting clients far from quest helper to "
            "force NPC respawn."
        )

        for c in clients:
            try:
                await c.teleport(target)
    ...
    async def unstick_missing_quest_npc(self, clients: list[Client]):

        leader = self.current_leader_client

        prev = self._npc_missing_iterations

        # combat means an entity DID stream in - clear any prior misses so a
        # post-fight free window can't push the counter to threshold
        if leader.entity_detect_combat_status:
            if prev:
                logger.debug(f"[unstick] reset: in combat (was {prev})")

            self._npc_missing_iterations = 0

            return

        if not await is_free_leader_questing(leader):
            logger.debug(f"[unstick] skip: leader not free (counter held at {prev})")

            return

        try:
            pos = await leader.body.position()

            quest_xyz = await leader.quest_position.position()

        except Exception as exc:
            logger.debug(
                f"[unstick] skip: quest_position read failed "
                f"({type(exc).__name__}); counter held at {prev}"
            )

            return

        helper_set = calc_Distance(quest_xyz, XYZ(0.0, 0.0, 0.0)) > 1

        dist_to_helper = calc_Distance(pos, quest_xyz)

        at_helper = helper_set and dist_to_helper < 1500.0

        if not at_helper:
            if prev:
                logger.debug(
                    f"[unstick] reset: not at helper "
                    f"(dist={dist_to_helper:.0f}, was {prev})"
                )
        ...
        # defeat quests point the helper at mob spawns, which never raise
        # ``npc_range_path`` (that's interactable-only). the streaming
        # reset is for interactable NPCs/sigils - running it for mobs
        # would drag the leader away mid-rotation. skip outright; mob
        # spawn issues are the quest engine's own problem
        objective = (await self.get_truncated_quest_objectives(leader)).lower()

        if "defeat" in objective:
            self._npc_missing_iterations = 0

            return

        if await is_visible_by_path(leader, npc_range_path):
            if prev:
                logger.debug(f"[unstick] reset: npc_range_path visible (was {prev})")

            self._npc_missing_iterations = 0

            return

        self._npc_missing_iterations += 1

        logger.debug(
            f"[unstick] miss #{self._npc_missing_iterations} "
            f"(objective={objective[:60]!r}, dist={dist_to_helper:.0f})"
        )

        if self._npc_missing_iterations < 2:
            return

        logger.debug(
            f"Quest NPC hasn't streamed in for {self._npc_missing_iterations} "
            "iterations - running streaming reset."
        )

        await self.streaming_reset(clients)
    ...
    async def determine_solo_zone(self) -> bool:
        ...
        sprinter = EntityClient(self.current_leader_client)
    ...
    async def get_questing_clients(self) -> list[Client]:
        ...
        quest_objective_leader = await self.get_truncated_quest_objectives(
    ...
    async def get_client_quests(
        self, questing_clients: list[Client]
    ) -> dict[Client, str]:
                    ...
                    if await is_free(c) and not c.entity_detect_combat_status:
                        ...
                        async with c.mouse_handler:
                            await teleport_to_friend_from_list(
                                c, name=self.current_leader_client.wizard_name
                            )
                        ...
                        if c_zone != leader_zone:
                            async with c.mouse_handler:
                                try:
                                    await safe_wait_for_zone_change(
                                        c, handle_hooks_if_needed=False
                                    )
                                    ...
                                    solo_zone = (
                                        await self.current_leader_client.zone_name()
                                    )
                                    ...
                                    logger.exception(
                                        "LoadingScreenNotFound during follow-up teleport"
                                    )
                                    while True:
                            ...
                            if await is_visible_by_path(
                                c, friend_is_busy_and_dungeon_reset_path
                            ):
                                async with c.mouse_handler:
    ...
    async def zone_recorrect_friend_tp(
        self, maybe_solo_zone: bool, gear_switching_in_solo_zones=False
    ):
        ...
        if len(self.clients) > 1:
            while not await self.followers_in_correct_zone() or maybe_solo_zone:
                clients_in_solo_zone, solo_zone = await self.friend_teleport(
                    maybe_solo_zone
                )
                ...
                if len(clients_in_solo_zone) > 0:
                    for solo_client in clients_in_solo_zone:
                        logger.debug(
                            "Client "
                            + solo_client.title
                            + " is in solo zone - questing alone"
                        )
                    ...
                    if gear_switching_in_solo_zones:
                        logger.debug(
                            "Switching to second equipment set on all clients."
                        )
                    ...
                    while len(clients_in_solo_zone) > 0 and solo_zone is not None:
                        solo_zone_task = asyncio.create_task(
                            solo_zone_questing_loop(
                                clients_in_solo=clients_in_solo_zone, zone=solo_zone
                            )
                        )
                        ...
                        logger.debug(
                            "Clients may have left the solo zone - attempting to teleport to leader."
                        )

                        clients_in_solo_zone, solo_zone = await self.friend_teleport(
                            maybe_solo_zone=True
                        )
                    ...
                    if maybe_solo_zone:
                        logger.debug(
                            "Some clients appear to still be in the solo zone."
                        )
                    ...
                    if gear_switching_in_solo_zones:
                        logger.debug(
                            "Switching back to first equipment set on all clients."
                        )
                        ...
                        await asyncio.gather(
                            *[
                                ...
                                for c in initial_clients_in_solo_zone
                            ]
                        )
    ...
    async def heal_and_handle_potions(self):
        ...
        # only refill for clients that have opted into buying potions. without
        # this gate, the leader marks every hooked client's position and drags
        # them to the bazaar whenever ANY client is below one charge - even
        # ones whose owner set `buy_potions = False` to keep them parked
        for p in self.clients:
            if not getattr(p, "buy_potions", False):
                continue
            if (
                await p.stats.potion_charge() < 1.0
                and await p.stats.reference_level() >= 6
            ):
                any_client_needs_potions = True

        if any_client_needs_potions:
            opted_in = [c for c in self.clients if getattr(c, "buy_potions", False)]

            for p in opted_in:
                if await p.zone_name() != "WizardCity/WC_Hub":
                    await guarantee_teleport_mark(p)

            await asyncio.gather(
                *[
                    refill_potions(
                        c, mark=False, recall=False, original_zone=await c.zone_name()
                    )
                    for c in opted_in
                ]
            )
            ...
            await asyncio.gather(
                *[self.gather_clients_from_potion_buy(c) for c in opted_in]
            )
    ...
    async def collect_wisps(self, p: Client):
        ...
        if await is_free(p):
            if (
                await is_potion_needed(p)
                and await p.stats.current_mana() > 1
                and await p.stats.current_hitpoints() > 1
            ):
    ...
    async def guarantee_use_potion(self, p: Client):
        ...
        if await is_free(p):
            if await is_potion_needed(p, minimum_mana=16):
                ...
                while (
                    await p.stats.potion_charge() == original_potion_count
                    and original_potion_count >= 1
                ):
    ...
    async def gather_clients_from_potion_buy(self, p: Client):
        ...
        while True:
            try:
                ...
                await safe_wait_for_zone_change(
                    p, name="WizardCity/WC_Hub", handle_hooks_if_needed=True
                )
    ...
    async def find_quest_zone_area_name(
        self, client: Client, door_locations: list
    ) -> Optional[str]:
    ...
    async def new_world_doors(self, client: Client) -> bool:
        ...
        if "Streamportal" in await self.read_spiral_door_title(client):
            location = await self.find_quest_zone_area_name(
                client, streamportal_locations
            )
    ...
    async def handle_spiral_navigation(self):
        ...
        if await self.new_world_doors(self.current_leader_client):
            ...
            leader_world = (await self.current_leader_client.zone_name()).split("/", 1)[
                0
            ]
        ...
        for code_generator in self.clients:
            for code_redeemer in self.clients:
                if code_generator.process_id != code_redeemer.process_id:
                    friend_already_in_list = await check_for_friend_in_list(
                        code_generator, code_redeemer.wizard_name
                    )
    ...
    async def auto_friend_all_wizards(self):
        ...
        await self.current_leader_client.send_key(Keycode.W, 0.5)
        ...
        for requester in self.clients:
            ...
            await requester.teleport(
                XYZ(x=-1884.0, y=-2328.0, z=0.0), yaw=0.7693982720375061
            )
            ...
            for acceptor in self.clients:
                if requester.process_id != acceptor.process_id:
                    friend_already_in_list = await check_for_friend_in_list(
                        requester, acceptor.wizard_name
                    )

                    if not friend_already_in_list:
                        original_acceptor_location = await acceptor.body.position()

                        await acceptor.teleport(
                            await requester.body.position(),
                            yaw=await requester.body.yaw(),
                        )
                        ...
                        async with requester.mouse_handler:
                            ...
                            for i in range(5):
                                await requester.mouse_handler.click(
                                    x=int(center_x), y=int(center_y), sleep_duration=0.3
                                )
                            ...
                            for i in range(2):
                                if await is_visible_by_path(
                                    requester, add_remove_friend_path
                                ):
                                    await click_window_by_path(
                                        requester, add_remove_friend_path
                                    )
                            ...
                            for i in range(2):
                                if await is_visible_by_path(
                                    requester, confirm_send_friend_request
                                ):
                                    await asyncio.sleep(0.2)
                            ...
                            for i in range(2):
                                while not await is_visible_by_path(
                                    acceptor, confirm_accept_friend_request
                                ):
                                    await asyncio.sleep(0.1)
                            ...
                            async with acceptor.mouse_handler:
                                ...
                                for i in range(5):
                                    try:
                                        await click_window_by_path(
                                            requester,
                                            close_real_friend_list_button_path,
                                        )
    ...
    async def determine_new_leader_and_followers(
        self,
        client_quests: dict,
        questing_clients: list[Client],
        follower_clients: list[Client],
    ) -> tuple[list[Client], dict[Client, str]]:
        ...
        if len(client_quests) > 0:
            for c in questing_clients:
                if c in client_quests:
                    if await self.get_truncated_quest_objectives(
                        c
                    ) != client_quests.get(c):
                        client_quests.pop(c)
            ...
            if len(client_quests) == 0:
                if self.current_leader_client.title != self.client.title:
                    logger.debug(
                        "Clients caught up - resetting leader to client "
                        + self.client.title
                    )
                ...
                client_quests = await self.get_client_quests(
                    questing_clients=questing_clients
                )

            elif len(client_quests) < original_length:
                logger.debug(
                    "client(s) fell behind - new leader "
                    + list(client_quests)[0].title
                    + " assigned"
                )
    ...
    async def correct_dungeon_desync(self, follower_clients):
        ...
        async def teleport_to_friend_from_list_wizard_leader_name_mouse_handler(client):
            ...
            async with client.mouse_handler:
                await teleport_to_friend_from_list(
                    client, name=self.current_leader_client.wizard_name
                )

        await asyncio.gather(
            *[
                teleport_to_friend_from_list_wizard_leader_name_mouse_handler(c)
        ...
        if teleport_banned_zone:
            logger.debug(
                "Friend teleport correction for dungeon desync failed - sending all clients to hub and retrying teleport"
            )
    ...
    async def auto_collect_rewrite(self, client: Client):
        ...
        chunk_cords = await self.get_zone_chunks()

        for points in chunk_cords:
            points = XYZ(points.x, points.y, points.z - 550)

            await client.teleport(points)
            ...
            for e in entities:
                try:
                    ...
                    display_name = await self.client.cache_handler.get_langcode_name(
                        display_name_code
                    )

                    match = fuzz.token_sort_ratio(
                        display_name.lower(), quest_item_list.lower()
                    )

                    if match > 80:
                        while (
                            not await is_free(self.client)
                            or self.client.entity_detect_combat_status
                        ):
                            await asyncio.sleep(0.1)

                        if await self.collect_entity(e):
                            return
            ...
            for e in entities:
                try:
                    ...
                    e_name = str(await temp.object_name())

                    if e_name not in entities_to_skip:
                        ...
                        edit_name2 = "".join(
                            [i for i in edited_name if not i.isdigit()]
                        )
    ...
    async def collect_entity(self, e: DynamicClientObject) -> bool:
        ...
        for _ in range(2):
            can_Teleport = await self.is_position_safe(xyz)

            if can_Teleport:
                safe_location = await self.client.body.position()

                try:
                    await navmap_tp(self.client, xyz)
                except Exception:
                    logger.exception("navmap_tp failed")
                ...
                if await is_visible_by_path(self.client, path=npc_range_path):
                    for i in range(5):
                        await asyncio.gather(
                            *[p.send_key(Keycode.X, 0.1) for p in self.clients]
                        )
    ...
    async def dungeon_recall(self, p: Client) -> Optional[bool]:
        ...
        if dungeon_recalled:
            while original_zone == await p.zone_name():
                ...
                dungeon_full = await click_window_until_closed(
                    p, friend_is_busy_and_dungeon_reset_path
                )
    ...
    async def auto_pet_questing(
        self, questing_clients: list[Client], ignore_pet_level_up, play_dance_game
    ):
        ...
        if auto_pet_on:
            ...
            if any_client_leveled_up:
                logger.debug(
                    "One or more questing clients leveled up - training pets on all questing clients."
                )

                await asyncio.gather(
                    *[
                        auto_pet(c, ignore_pet_level_up, play_dance_game, questing=True)
                        for c in self.clients
                    ]
                )

    async def handle_zone_correction(
        self,
        maybe_solo_zone: bool,
        questing_friend_tp: bool,
        gear_switching_in_solo_zones: bool,
    ):
        ...
        if not await self.followers_in_correct_zone() or maybe_solo_zone:
            logger.debug(
                "Clients may be in wrong zone - attempting to correct with X press"
            )
            ...
            await asyncio.gather(
                *[exit_menus(c, end_of_loop_paths) for c in self.clients]
            )
        ...
        if not await self.followers_in_correct_zone() or maybe_solo_zone:
            if not questing_friend_tp:
                ...
                maybe_solo_zone = await self.zone_recorrect_friend_tp(
                    maybe_solo_zone, gear_switching_in_solo_zones
                )

                await asyncio.sleep(2.0)

    async def handle_dungeon_entry(
        self, questing_friend_tp: bool, follower_clients: list[Client]
    ):
        ...
        if questing_friend_tp:
            ...
            if num_visible_clients == len(self.clients):
                ...
                logger.debug(
                    "One or more clients was separated from the group - teleporting all to leader"
                )
    ...
    async def handle_npc_talking_quests(
        self,
        talking_client: Client,
        present_clients: list[Client],
        settle_window: float = 3.0,
    ):
        ...
        # ``settle_window`` is how long we keep watching for a *follow-up*
        # dialog/menu after the conversation ends (the timer resets each time
        # one appears). the multi-client leader path keeps the generous 3s
        # default; solo questing passes a tighter value so a single-stage talk
        # returns almost immediately, matching a hand-written interact()
        while time.time() < start_time + settle_window:
    ...
    async def teleport_to_quest(
        self, hitting_client: str, follower_clients: list[Client]
    ):
        ...
        if await is_free_leader_questing(self.current_leader_client):
            leader_client_objective_xyz = (
                await self.current_leader_client.quest_position.position()
            )

            leader_objective = await self.get_truncated_quest_objectives(
                self.current_leader_client
            )
            ...
            if "defeat" in leader_objective.lower():
                    ...
                    if hitting_client in self.current_leader_client.title:
                        ...
                        for c in follower_clients:
                            if proxy_leader_client is None:
                                if await self.get_truncated_quest_objectives(
                                    c
                                ) == await self.get_truncated_quest_objectives(
                                    self.current_leader_client
                                ):
                ...
                logger.debug(
                    "Leader "
                    + self.current_leader_client.title
                    + " on defeat quest - staggering teleports"
                )
                ...
                crossed_gate = await walk_quest_gate_if_cross_zone(
                    proxy_leader_client, leader_client_objective_xyz
                )

                if not crossed_gate:
                    await proxy_leader_client.teleport(leader_client_objective_xyz)

                    async def proxy_leader_moved():
                        return (
                            calc_Distance(
                                location_before_sendback,
                                await proxy_leader_client.body.position(),
                            )
                            >= 20
                        )

                    await wait_until(proxy_leader_moved, timeout=1.0)
                    ...
                    if distance < 20:
                        logger.debug(
                            "client "
                            + proxy_leader_client.title
                            + " collided on initial teleport"
                        )

                        await navmap_tp(
                            client=proxy_leader_client,
                            xyz=leader_client_objective_xyz,
                            leader_client=self.current_leader_client,
                        )
                ...
                detected_dungeon = await self.detected_interact_from_popup(
                    proxy_leader_client
                )

                if (
                    await proxy_leader_client.zone_name() != zone_before_teleport
                    or detected_dungeon
                ):
                    logger.debug(
                        "leader zone changed or interactible reached - syncing all clients"
                    )
                    ...
                    try:
                        await asyncio.gather(
                            *[
                                ...
                                for c in followup_teleport_clients
                            ]
                        )

                    except Exception:
                        logger.exception("follow-up teleport gather failed")

                elif (
                    proxy_leader_is_questing
                    and leader_objective
                    != await self.get_truncated_quest_objectives(proxy_leader_client)
                ):
                    pass
                    ...
                    sprinter = EntityClient(proxy_leader_client)

                    async def proxy_leader_in_combat():
                        return proxy_leader_client.entity_detect_combat_status
                    ...
                    while not proxy_leader_client.entity_detect_combat_status:
                        ...
                        await wait_until(proxy_leader_in_combat, timeout=5.0)

                    while proxy_leader_client.entity_detect_combat_status:
                        await asyncio.sleep(0.1)

            else:
                leader_crossed = await walk_quest_gate_if_cross_zone(
                    self.current_leader_client, leader_client_objective_xyz
                )

                if not leader_crossed:
                    await asyncio.gather(
                        *[
                            navmap_tp(
                                p,
    ...
    async def handle_normal_quests(
        self, follower_clients: list[Client], questing_friend_tp: bool
    ):

        await asyncio.gather(
            *[safe_click_window(c, cancel_chest_roll_path) for c in self.clients]
        )

        await asyncio.gather(
            *[safe_click_window(c, exit_dungeon_path) for c in self.clients]
        )
        ...
        if await is_free_leader_questing(self.current_leader_client):
            ...
            if (
                await is_visible_by_path(self.current_leader_client, npc_range_path)
                and calc_Distance(leader_client_objective_xyz, current_pos) < 750.0
            ):
                ...
                if "to enter" in sigil_msg_check.lower():
                    while "to enter" in sigil_msg_check.lower():
                        ...
                        sigil_msg_check = await self.read_popup(
                            self.current_leader_client
                        )

                    await self.handle_dungeon_entry(
                        questing_friend_tp, follower_clients
                    )
                    ...
                    if "to talk" in msg:
                        ...
                        "magic raft" in msg or "to ride" in msg or "to teleport" in msg
                    ):
                    ...
                    async def leader_is_loading():
                        return await self.current_leader_client.is_loading()

                    await wait_until(leader_is_loading, timeout=2.0)
                    ...
                    await wait_until(
                        lambda: is_visible_by_path(
                            self.current_leader_client, spiral_door_teleport_path
                        ),
                        timeout=0.75,
                    )
                ...
                if "Photomance" in quest_objective:
                    await asyncio.gather(
                        *[p.send_key(key=Keycode.Z, seconds=0.1) for p in self.clients]
                    )
    ...
    async def handle_repeated_normal_quest_failures(
        self,
        last_leader_pid,
        last_leader_zone: str,
        iterations_since_last_quest_change: int,
    ):

        if (
            last_leader_pid == self.current_leader_client.process_id
            and last_leader_zone == await self.current_leader_client.zone_name()
        ):
            if iterations_since_last_quest_change >= 5:
                ...
                await asyncio.gather(
                    *[
                        p.teleport(XYZ(location.x + 500, location.y, location.z - 1500))
                        for p in self.clients
                    ]
                )
    ...
    async def hardcoded_collect(
        self, incompatible_hardcoded_quests, truncated_quest_obj: str
    ):
        ...
        for c in self.clients:
            if c.process_id != self.current_leader_client.process_id:
                ...
                while current_quest == truncated_quest_obj:
                    ...
                    current_quest = (
                        await self.get_truncated_quest_objectives(c)
                    ).lower()
        ...
        while current_quest == truncated_quest_obj:
            ...
            current_quest = (
                await self.get_truncated_quest_objectives(self.current_leader_client)
            ).lower()

        while (
            not await is_free(self.current_leader_client)
            or self.current_leader_client.entity_detect_combat_status
        ):
            await asyncio.sleep(1.0)

        if (
            await is_free(self.current_leader_client)
            and not self.current_leader_client.entity_detect_combat_status
        ):
    ...
    async def handle_collect_quests(self):
        ...
        leader_obj = await self.get_truncated_quest_objectives(
            self.current_leader_client
        )
    ...
    async def bring_clients_to_same_location(
        self, questing_friend_tp: bool, gear_switching_in_solo_zones: bool
    ):
        ...
        if questing_friend_tp:
            ...
            await self.zone_recorrect_friend_tp(
                maybe_solo_zone=maybe_solo_zone,
                gear_switching_in_solo_zones=gear_switching_in_solo_zones,
            )

    async def initial_auto_pet(
        self,
        energy_info,
        questing_clients: list[Client],
        ignore_pet_level_up: bool,
        play_dance_game: bool,
    ):
        ...
        if auto_pet_on:
            ...
            if all_high_energy:
                ...
                logger.debug(
                    "All questing clients have high energy, training pets on all clients."
                )
    ...
    async def auto_quest_leader(
        ...
        if questing_friend_tp or self.current_leader_client.auto_pet_status:
            ...
            await asyncio.gather(
                *[set_wizard_name_from_character_screen(c) for c in self.clients]
            )

            energy_info = await asyncio.gather(
                *[
                    return_wizard_energy_from_character_screen(c)
                    for c in questing_clients
                ]
            )

            await asyncio.gather(
                *[self.close_character_screen(c) for c in self.clients]
            )

        if self.current_leader_client.auto_pet_status:
            await self.initial_auto_pet(
                energy_info, questing_clients, ignore_pet_level_up, play_dance_game
            )

        await self.bring_clients_to_same_location(
        ...
        while self.client.questing_status:
            ...
            await self.auto_pet_questing(
                questing_clients, ignore_pet_level_up, play_dance_game
            )

            (
                follower_clients,
                client_quests,
            ) = await self.determine_new_leader_and_followers(
                client_quests, questing_clients, follower_clients
            )

            await self.handle_zone_correction(
                maybe_solo_zone, questing_friend_tp, gear_switching_in_solo_zones
            )
            ...
            if await is_free_leader_questing(self.current_leader_client):
                ...
                if distance > 1:
                    await self.handle_repeated_normal_quest_failures(
                        last_leader_pid,
                        last_leader_zone,
                        iterations_since_last_quest_change,
                    )

                    await self.teleport_to_quest(hitting_client, follower_clients)

                    await self.handle_normal_quests(
                        follower_clients, questing_friend_tp
                    )

                    await self.unstick_missing_quest_npc(self.clients)

                else:

                    async def leader_quest_pos_valid():
                        qp = await self.current_leader_client.quest_position.position()
                        return calc_Distance(qp, XYZ(0.0, 0.0, 0.0)) > 1

                    await wait_until(leader_quest_pos_valid, timeout=3.0)
                    ...
                    if distance < 1:
                        await get_quest_name(self.current_leader_client)
                        ...
                        incompatible_hardcoded_quests = {
                            "find submarine parts in the floating land": [
                                40,
                                XYZ(
                                    x=-17412.53515625,
                                    y=10505.794921875,
                                    z=-429.19927978515625,
                                ),
                                XYZ(
                                    x=-17088.447265625,
                                    y=12284.244140625,
                            ...
                            "catch vonda fish in pitch black lake": [
                                3,
                                XYZ(
                                    x=-10030.9912109375,
                                    y=-7925.51953125,
                            ...
                            "break mining equipment in tyrian gorge": [
                                25,
                                XYZ(
                                    x=-2529.12890625,
                                    y=-3015.832763671875,
                        ...
                        if any(
                            map(
                                truncated_quest_obj.__contains__,
                                incompatible_hardcoded_quests.keys(),
                            )
                        ):
                            logger.debug("Hardcoded collect")

                            await self.hardcoded_collect(
                                incompatible_hardcoded_quests, truncated_quest_obj
                            )
                        ...
                        logger.debug(
                            "False collect quest detected.  Quest position: "
                            + str(distance)
                        )
            ...
            if (
                leader_full_current_quest == leader_last_full_quest
                and last_leader_pid == self.current_leader_client.process_id
                and last_leader_zone == await self.current_leader_client.zone_name()
            ):
                iterations_since_last_quest_change += 1

            else:
                leader_last_full_quest = await get_quest_name(
                    self.current_leader_client
                )
    ...
    async def auto_quest_solo(
        self, auto_pet_disabled=False, ignore_pet_level_up=False, play_dance_game=False
    ):

        if await is_free(self.client):
            if (
                await is_potion_needed(self.client)
                and await self.client.stats.current_mana() > 1
                and await self.client.stats.current_hitpoints() > 1
            ):
            ...
            if self.client.auto_pet_status and not auto_pet_disabled:
                if (
                    self.client.character_level
                    < await self.client.stats.reference_level()
                ):
                    logger.debug(
                        "Client " + self.client.title + " leveled up - training pet."
                    )

                    await auto_pet(
                        self.client, ignore_pet_level_up, play_dance_game, questing=True
                    )
            ...
            if distance > 1:
                ...
                # shared engine behind client:tp_to_quest() - cross-zone gate
                # walk, same-zone teleport with snap-out / rejection recovery,
                # stale-helper poll, persona stream-nudge, and the
                # accidental-sub-instance walk-back. reads the helper itself,
                # so the quest_xyz above is only used for the distance gate
                await run_tp_to_quest(self.client)

                await self.handle_questing_zone_change()

                # instead of a blind 0.5s dwell, poll for the interact popup
                # so we proceed the instant it appears. goals with nothing to
                # click (waypoint/collect) still fall through after the short
                # cap - no slower than before, much faster when there IS an
                # interactable waiting
                await wait_until(
                    lambda: is_visible_by_path(self.client, npc_range_path),
                    timeout=0.5,
                    interval=0.05,
                )
                ...
                if (
                    await is_visible_by_path(self.client, npc_range_path)
                    and calc_Distance(quest_xyz, current_pos) < 750.0
                ):
                    ...
                    if "to enter" in sigil_msg_check.lower():
                        ...
                        await self.handle_npc_talking_quests(
                            self.client, [self.client], settle_window=1.0
                        )
                        ...
                        await wait_until(
                            lambda: is_visible_by_path(
                                self.client, spiral_door_teleport_path
                            ),
                            timeout=0.75,
                        )
                ...
                if await is_visible_by_path(self.client, missing_area_path):
                    while not await is_visible_by_path(
                        self.client, missing_area_retry_path
                    ):
                        await asyncio.sleep(0.1)

                    await click_window_by_path(
                        self.client, missing_area_retry_path, True
                    )

            else:

                async def solo_quest_pos_valid():
                    qp = await self.client.quest_position.position()
                    return calc_Distance(qp, XYZ(0.0, 0.0, 0.0)) > 1

                await wait_until(solo_quest_pos_valid, timeout=3.0)
    ...
    async def auto_quest(self, ignore_pet_level_up: bool, play_dance_game: bool):
        ...
        # tight loop mirroring a hand-written `tp_to_quest()` + `interact()`
        # script: act the instant the client is free instead of eating a fixed
        # one-second delay every iteration. combat / dialog / loading are owned
        # by other tasks, so when we're not free we just poll briefly and yield
        # rather than running a no-op questing pass
        while self.client.questing_status:
            ...
            await self.auto_quest_solo(
                ignore_pet_level_up=ignore_pet_level_up, play_dance_game=play_dance_game
            )

            await self.unstick_missing_quest_npc([self.client])

            # brief settle between active passes. keeps the unstick miss-counter
            # near its previous cadence - a far-NPC streaming reset shouldn't
            # fire on a hair trigger - without reinstating the full 1s stall
            await asyncio.sleep(0.3)
...
async def is_free_leader_questing(client: Client):
    ...
    return not any(
        [
            ...
            client.entity_detect_combat_status,
            (dialogue_text != ""),
        ]
    )
