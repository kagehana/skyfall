    ...
    async def cast(
        ...
        *,
        ...
        if isinstance(target, CombatCard):
            ...
            confirm_window = (
                await self.combat_handler.client.root_window.get_windows_with_name(
                    "ConfirmTargetsWindow"
                )
            )[0]
            if await confirm_window.is_visible():
                try:
                    await (
                        self.combat_handler.client.mouse_handler.click_window_with_name(
                            "ConfirmTargetsConfirm"
                        )
                    )
                except ValueError:
                    pass
