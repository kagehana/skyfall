...
class CombatHandler:
    ...
    async def _get_card_windows(self):
        ...
        spell_checkbox_windows = [
            x for x in spell_checkbox_windows if await x.name() != "PetCard"
        ]
    ...
    async def get_damaging_aoes(self, *, check_enchanted: bool = None):
        ...
        async def _pred(card: CombatCard):
            ...
            for effect in effects:
                ...
                if any(
                    substr in effect_type.lower() for substr in ("variable", "random")
                ):
    ...
    async def get_client_member(
        self, *, retries: int = 5, sleep_time: float = 0.5
    ) -> CombatMember:
        """
    ...
    async def attempt_willcast(self, *, on_member: str = None, on_client: bool = False):
        """
        ...
        try:
            spell_checkbox_windows = (
                await self.client.root_window.get_windows_with_type("SpellCheckBox")
            )
            card = CombatCard(
                self,
                ([x for x in spell_checkbox_windows if await x.name() == "PetCard"])[0],
            )
