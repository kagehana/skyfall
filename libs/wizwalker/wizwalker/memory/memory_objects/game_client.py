...
class GameClient(MemoryObject):
    ...
    async def elastic_camera_controller(
        self,
    ) -> Optional[DynamicElasticCameraController]:
    ...
    async def write_selected_camera_controller(
        self, selected_camera_controller: Union[CameraController, int]
    ):
        """
        ...
        if isinstance(selected_camera_controller, CameraController):
            selected_camera_controller = (
                await selected_camera_controller.read_base_address()
            )
        ...
        await self.write_value_to_offset(
            offset, selected_camera_controller, Primitive.uint64
        )
