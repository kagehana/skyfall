...
class SpellEffect(PropertyClass):
    ...
    async def maybe_effect_list(
        ...
        for addr in await self.read_shared_linked_list(224):
            effect = await cast_effect_variant(
                DynamicSpellEffect(self.hook_handler, addr)
            )
...
class HangingConversionSpellEffect(DynamicSpellEffect):
    ...
    async def specific_effect_types(
        self,
    ) -> list[
        SpellEffects
    ]:  # TODO: missing a write function, doesn't really matter -slack
        results = []
    ...
    async def write_scale_source_effect_percent(
        self, scale_source_effect_percent: float
    ):
        await self.write_value_to_offset(
            276, scale_source_effect_percent, Primitive.float32
        )
