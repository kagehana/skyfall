...
class MemoryObject(MemoryReader):
    ...
    async def write_value_to_offset(
        self, offset: int, value: Any, data_type: Primitive
    ):
        base_address = await self.read_base_address()
    ...
    async def read_vector(
        self, offset: int, size: int = 3, data_type: Primitive = Primitive.float32
    ):
    ...
    async def write_vector(
        ...
        value: tuple,
        size: int = 3,
        data_type: Primitive = Primitive.float32,
    ):
        type_str = data_type.value.format.replace("<", "")
    ...
    async def read_hashset_basic(
        self, offset: int, primitive_type: Primitive
    ) -> set[int]:
        result = set()
        root = DynamicMemoryObject(
            self.hook_handler,
            await self.read_value_from_offset(offset, Primitive.uint64),
        )
        stack = [
            DynamicMemoryObject(
                self.hook_handler,
                await root.read_value_from_offset(8, Primitive.uint64),
            )
        ]
        while len(stack) > 0:
            node = stack.pop()
            ...
            stack.extend(
                [
                    DynamicMemoryObject(
                        self.hook_handler,
                        await node.read_value_from_offset(0x00, Primitive.uint64),
                    ),
                    DynamicMemoryObject(
                        self.hook_handler,
                        await node.read_value_from_offset(0x10, Primitive.uint64),
                    ),
                ]
            )
