...
class HookHandler(MemoryReader):
    ...
    def __init__(self, process: pymem.Pymem, client):
        ...
        # CombatActions resolves game-message function pointers and sends
        # MSG_COMBATMOVE/etc. via shellcode. Activated lazily by
        # activate_combat_actions(); None until then.
        self.combat_actions: CombatActions | None = None
    ...
    async def close(self):
        # Each unhook restores patched bytes in the target process. If one
        # fails (transient write_bytes error, window flicker, etc.) we MUST
        # still attempt the rest — otherwise the leftover detours rot the
        # process and the next hook attempt's pattern scan can't match.
        if self.combat_actions is not None:
            try:
                await self.combat_actions.cleanup()
            except Exception as exc:
                logger.error(
                    f"combat_actions.cleanup failed: {type(exc).__name__}: {exc}"
                )
            self.combat_actions = None

        for hook_type, hook in list(self._active_hooks.items()):
            try:
                await hook.unhook()
            except Exception as exc:
                logger.error(
                    f"unhook of {hook_type.__name__} failed: {type(exc).__name__}: {exc}"
                )
        ...
        try:
            ...
            logger.error(
                f"_rewrite_autobot failed during close: {type(exc).__name__}: {exc}"
            )
    ...
    async def activate_combat_actions(self):
        """
        Resolve game-message function addresses and allocate the persistent
        data block used by the packet-level combat API. After this returns,
        ``self.combat_actions`` is a usable :class:`CombatActions` instance.
        Idempotent — re-activating reuses the existing instance.
        """
        if self.combat_actions is not None and self.combat_actions._resolved:
            return
        if self.combat_actions is None:
            self.combat_actions = CombatActions(self)
        await self.combat_actions.setup()

    async def deactivate_combat_actions(self):
        if self.combat_actions is not None:
            await self.combat_actions.cleanup()
            self.combat_actions = None
