...
class Client:
    ...
    async def get_base_entity_list(self):
        ...
        return await maybe_wait_for_any_value_with_timeout(
            partial(_impl, self), sleep_time=0.05, timeout=3.0
        )
    ...
    async def activate_hooks(
        ...
        # Combat-action shellcode: optional. If pattern scan fails on a
        # different game version the rest of the bot still works.
        try:
            await self.hook_handler.activate_combat_actions()
        except Exception as exc:
            logger.warning(
                f"combat_actions failed to activate ({type(exc).__name__}: {exc}); "
                f"send_combat_* will be unavailable"
            )

    # ------------------------------------------------------------------
    # Packet-level combat API (no UI interaction)
    # Requires ``activate_hooks`` to have succeeded.
    # ------------------------------------------------------------------

    def _require_combat_actions(self):
        ca = self.hook_handler.combat_actions
        if ca is None:
            raise RuntimeError(
                "combat_actions not activated — call activate_hooks() first"
            )
        return ca

    async def send_combat_pass(self):
        """Pass the current combat turn (MSG_COMBATMOVE, MoveType=Pass)."""
        await self._require_combat_actions().pass_turn()

    async def send_combat_flee(self):
        """Flee combat entirely (MSG_COMBATMOVE, MoveType=Flee)."""
        await self._require_combat_actions().flee()

    async def send_combat_spell(self, hand_index: int, target: int):
        """Cast spell at hand_index against target subcircle (0–7)."""
        await self._require_combat_actions().cast_spell(hand_index, target)

    async def send_combat_enchant(self, enchant_index: int, target_index: int):
        """Apply the enchant card at enchant_index to the spell at target_index."""
        await self._require_combat_actions().enchant_spell(
            enchant_index, target_index
        )

    async def send_combat_discard(self, hand_index: int):
        """Discard the card at hand_index from hand."""
        await self._require_combat_actions().discard_card(hand_index)
    ...
    async def send_combat_fusion(
        ...
        """Fuse two cards. ``fused_spell_id=0`` lets the server resolve."""
        await self._require_combat_actions().fuse_spells(
            primary_index, secondary_index, fused_spell_id
        )

    async def send_combat_draw(self):
        """Draw a treasure card from the TC deck (MSG_COMBATDRAW)."""
        await self._require_combat_actions().draw_card()

    async def send_pet_willcast(self, spell_name: str, target: int):
        """Trigger pet may-cast spell by name."""
        await self._require_combat_actions().pet_willcast(spell_name, target)
    ...
    async def close(self):
        ...
        # Both teardown steps must run independently. If _unpatch_movement_update
        # raises (e.g. its pattern scan fails because previous bytes were left
        # patched in), we still need hook_handler.close() to restore the rest
        # of the hooks — otherwise the next hook attempt finds the process
        # littered with detours and pattern scans drift until they fail.
        try:
            await self._unpatch_movement_update()
        except Exception as exc:
            logger.error(
                f"_unpatch_movement_update failed during close: "
        ...
        try:
            ...
            logger.error(
                f"hook_handler.close failed: {type(exc).__name__}: {exc}"
            )
    ...
    async def quest_manager(self) -> QuestClientManager:
        if not self._quest_client_manager_addr:
            ...
            self._quest_client_manager_addr = await self.hook_handler.read_typed(
                mov_instruction_addr + 7 + rip_offset, Primitive.uint64
            )
    ...
    async def backpack_space(self) -> tuple[int, int]:
        ...
        async def should_count_item(obj: CoreObject) -> bool:
            ...
            template = WizGameObjectTemplate(
                self.hook_handler, await template.read_base_address()
            )
            if await template.object_type() in [ObjectType.deed, ObjectType.structure]:
                return False
            exclusion_set = set(
                [make_string_id(x) for x in ["NotCounted", "Jewel", "Emote"]]
            )
    ...
    async def current_energy(self) -> int | None:
        ...
        """
        if (
            behavior := await self.client_object.search_behavior_by_name(
                "PetOwnerBehavior"
            )
        ) is not None:
